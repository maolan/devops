...
The crate builds a single `cdylib` (`maolan_plugins.so` / `.dll` / ``)
...
- **Website:** `site/` is a static HTML/CSS site deployed to `https://maolan.rs/`.
...
- Documentation for users is in `doc/` (a separate repository at the root level). Keep it in sync when changing
